...
export default function ActionButton({ className, onClick, children }) {
  ...
  const [wood, brick, sheep, wheat, ore] = currentResources;
  ...
  if(className.includes('buildRoad')) {
    ...
    isActionAllowed = wheat >= 2 && ore >= 3;
  } else if(className.includes('development')) {
    isActionAllowed = sheep >= 1 && wheat >= 1 && ore >= 1;
  }
  ...
  const canBuildCity = wheat >= 2 && ore >= 3;
  const canBuyDevCard = sheep >= 1 && wheat >= 1 && ore >= 1;
