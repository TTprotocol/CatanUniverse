...
const robOneResource = (victim, thief) => {
	...
	if (pool.length === 0) return { updatedFrom: victim, updatedTo: thief, stolen: null };
...
export const rollDiceSevenHandler = ({
	...
	const victims = afterDiscard.filter((p) => p.id !== currentPlayer.id && (p.buildings || []).some((b) => (b.adjacentTiles || []).includes(newTileId)));

	if (victims.length === 0) return; // 강탈 대상 없음

	// UI에서 고르게 하거나, 기본으로 첫 번째 피해자 선택
	const victimChosen = (typeof chooseVictimFn === "function" && chooseVictimFn(victims)) || victims[0];
	const { updatedFrom, updatedTo } = robOneResource(victimChosen, currentPlayer);
